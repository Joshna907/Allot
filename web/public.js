...
export const landingPage={
 ...
 async mount({root,alive}){
  ...
  async function book(){
   ...
   try{const parsed=await postJson("/api/parse",{text:DEFAULT_BOOK_TEXT});if(!alive())return;
    targets[0].innerHTML=`<div class="preview-head"><h2>Monthly demo book</h2><strong>${usd(parsed.totals.gross_usd)}</strong></div><p>${usd(parsed.totals.spend_usd)} allocated to three people</p><div class="recipient-list">${parsed.allocation.filter(r=>r.role==="spend").map(r=>`<div class="recipient-row"><div><strong>${esc(r.name)}</strong><span>${esc(r.city)}</span></div><strong>${usd(r.usd)}</strong></div>`).join("")}</div><p class="preview-held">${usd(parsed.totals.hold_usd)} excluded from preparation</p>`;
    targets[1].innerHTML=allocations(parsed.allocation);
   }catch(error){if(alive()){targets.forEach((target,index)=>target.innerHTML=errorState("Demo book unavailable",error.message,"retry-book-"+index));root.querySelectorAll('[id^="retry-book-"]').forEach(button=>button.onclick=book);}}
  }
